...
def _ensure_script_ready(bot, result: dict[str, Any], story_data: dict[str, Any], language_cfg: dict[str, Any], genre_key: str, format_mode: str) -> dict[str, Any]:
    """Require semantic narrative completeness before rendering; never enforce scene or word quotas."""
    ...
    if not valid:
        raise ValueError(
            f"Production script rejected after semantic fallback: {fallback_reason}"
        )
