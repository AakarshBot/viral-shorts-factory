"""Evidence-first, role-aware visual retrieval planner.

The planner deliberately separates three things:

1. factual entity identity (what the story is about),
2. the entity's role in this scene (participant, venue, person, product, etc.),
3. the smallest precise visual subject that should be searched.

Retrieval then uses a bounded query ladder which is never allowed to discard
identity or role. This is intentionally genre-agnostic: sports, politics,
technology, entertainment, history, geography and science use the same
resolution contract rather than accumulating entity-specific exceptions.
"""
from __future__ import annotations
import html
import re

VISUAL_RETRIEVAL_PLANNER_VERSION = "2026-09-17-v12-role-aware-cross-genre"
...
ORGANIZATION_SUFFIXES = ("board","council","federation","association","committee","corporation","company","university","institute","foundation","ministry","government","agency","authority","bank","club","party","commission","league","network","organization","organisation","parliament","congress")
...
VISUAL_CUES = {
 "cricket": {"t20i":"T20","t20":"T20","odi":"ODI","test match":"Test cricket","players":"players","player":"players","team":"team","squad":"players","batting":"batting","bowling":"bowling","celebration":"celebration","celebrating":"celebration","stadium":"stadium","ground":"ground","trophy":"trophy","final":"final","match":"match"},
...
def _explicit_team_context(text):
 return [country for country in NATIONAL_SPORTS_SIDES if re.search(r"\b"+re.escape(country.lower())+r"\b",text)]
...
def _team_role_is_explicit(text):
 return bool(re.search(r"\b(team|squad|players?|side|eleven|match|series|t20i|t20|odi|test|batting|bowling|face|faces|take on|takes on|vs|versus|against|playing|play)\b", text))

def _explicit_role(entity, text):
 """Return a strong scene role when the supplied scene explicitly supports one."""
 e=_clean(entity).lower(); t=text.lower()
 intent=t
 if any(x in intent for x in ("person portrait","portrait","headshot","person","president","prime minister","ceo","scientist","actor","coach","founder","minister","speaker")):
  return "PERSON"
 if any(x in intent for x in ("stadium venue","venue","stadium","ground","arena","landmark","map","location","geography")):
  return "LOCATION"
 if any(x in intent for x in ("organization","institution","company","corporation","agency","government","ministry","parliament","board","federation")):
  return "ORGANIZATION"
 if any(x in intent for x in ("product","device","phone","car","chip","console","prototype")):
  return "PRODUCT"
 if any(x in intent for x in ("scientific concept","concept","theory","mechanism","process","diagram")):
  return "CONCEPT"
 if any(x in intent for x in ("event","competition","tournament","summit","election","conference")):
  return "EVENT"
...
def _resolve_visual_subject(entity, seg, category=""):
 ...
 # Explicit scene role outranks generic lexical type, except that a generic
 # "location" request must not override a real sports participant relation.
 ...
 role=_explicit_role(entity,text)
 ...
 if _looks_like_location(entity): return entity
 if _looks_like_organization(entity): return entity
