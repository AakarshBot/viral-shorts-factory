...
def strict_fallback(story_data, language_cfg=None, genre_key="news", format_mode="regular"):
    """Delegate emergency narration to the canonical source-grounded script fallback."""
...
def _clean_script_result(script_data: dict, story_data: dict, format_mode: str = "regular") -> dict:
    ...
    if not valid:
        raise ValueError(f"Canonical script contract failed integrity validation: {reason}")
    return result
