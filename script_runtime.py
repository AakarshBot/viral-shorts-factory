...
_NARRATIVE_ROLE_RE = {
    "hook": re.compile(r"\b(?:announce|announced|announces|confirm|confirmed|confirms|launch|launched|reveal|revealed|reported|reports|decision|decided|latest|new|today|now|breaking)\b", re.IGNORECASE),
    "development": re.compile(r"\b(?:after|before|since|following|meanwhile|later|earlier|next|during|then|recently|update|updated|developed|development|changed|changes|moved|began|ended)\b", re.IGNORECASE),
    "context": re.compile(r"\b(?:because|background|previously|historically|compared|unlike|how|why|explains|explained|under|part of|according to|context|history|mechanism)\b", re.IGNORECASE),
    "consequence": re.compile(r"\b(?:impact|affect|affects|affected|effect|result|consequence|means|could|will|expected|response|next step|implication|matters|matter|likely|risk)\b", re.IGNORECASE),
}

_RETENTION_BAIT_RE = tuple(re.compile(pattern, re.IGNORECASE) for pattern in (
    r"\bwait\s+(?:until|till|for)\s+(?:the\s+)?end\b",
    r"\bwait\s+for\s+it\b",
    r"\b(?:stay|stick)\s+(?:around|with\s+me|with\s+us)\b",
    r"\bkeep\s+watching\b",
...
def _infer_narrative_role(scene, index, total):
    if not isinstance(scene, dict):
        return None

    explicit = str(scene.get("narrative_role") or "").strip().lower().replace("-", "_").replace(" ", "_")
    aliases = {
        "fact": "hook",
        "headline": "hook",
        "event": "hook",
    ...
    if explicit in _NARRATIVE_ROLE_RE:
        return explicit
    ...
    scores = {role: len(regex.findall(text)) for role, regex in _NARRATIVE_ROLE_RE.items()}
    best_role, best_score = max(scores.items(), key=lambda item: item[1])
    return best_role if best_score else ("context" if index % 2 == 0 else "development")


def assess_narrative_completeness(script_data):
    scenes = script_data.get("script", []) if isinstance(script_data, dict) else []
    if not scenes:
        return {"passed": False, "reason": "Script contains no narration scenes.", "roles": {}, "scene_roles": []}

    roles = {}
    scene_roles = []
    for index, scene in enumerate(scenes):
        if not isinstance(scene, dict):
            return {"passed": False, "reason": f"Scene {index + 1} is malformed.", "roles": roles, "scene_roles": scene_roles}
        if not str(scene.get("voiceover") or "").strip():
            return {"passed": False, "reason": f"Scene {index + 1} is empty.", "roles": roles, "scene_roles": scene_roles}
        role = _infer_narrative_role(scene, index, len(scenes))
        scene_roles.append(role)
        roles.setdefault(role, []).append(index + 1)

    required = set(_NARRATIVE_ROLE_RE)
    missing = sorted(required - set(roles))
    if missing:
        return {
            "passed": False,
            "reason": "Narrative is incomplete; it is missing distinct " + ", ".join(missing) + " beats.",
            "roles": roles,
...
def validate_content_density(script_data, story_data, format_mode):
    ...
    return True, "Passed narrative completeness, visual grounding and anti-filler checks"


def _extractive_script_fallback(story_data, language_cfg, genre_key, format_mode):
    """Emergency source-only path; never invent scenes or filler."""
            ...
            if isinstance(items, list):
                ...
                for item in items:
                    if isinstance(item, dict):
                        parts.extend(str(item.get(key) or "").strip() for key in ("title", "text", "summary"))
    ...
    sentences = [
        ...
        if sentence.strip() and not contains_retention_bait(sentence) and not _looks_like_filler(sentence)
    ]
    if not sentences:
        raise ValueError("Source-grounded fallback refused: no usable source narration.")
    ...
    if not valid:
        raise ValueError(f"Source-grounded fallback failed narrative completeness: {reason}")
    return result
...
def wrap_write_script(bot):
    ...
    def write_script(story_data, language_cfg, genre_key, conn, format_mode):
        ...
        if not ok:
            print(
                f"   [Script Fallback] Generated script rejected ({reason}). "
                "Trying source-grounded emergency fallback.",
                flush=True,
            )
            ...
            cleaned["originality_critique"] = {
                "score": None,
                "unsupported_claims": ["Source-grounded fallback is not eligible for public publication."],
                "exaggerations": [],
                "fixes": [],
                "provider": "fallback",
            }
        ...
        if not originality["passed"]:
            print(
                f"   [Script Originality] Meaningful source overlap detected in {len(originality['failures'])} scene(s); requesting one rewrite.",
                flush=True,
            )
