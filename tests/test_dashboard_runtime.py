...
def test_visual_dashboard_exposes_fresh_crawler_and_source_page_controls():
    ...
    assert "Automatic crawler diagnostics" in source
...
def test_script_review_allows_human_edit_above_historical_duration(monkeypatch):
    ...
    controller = DashboardWorkflowController(_Bot())
    controller.state.stage = "script_review"
    controller.state.run_id = "run-script-duration"
    controller.state.format_mode = "regular"
    controller.state.selected_story = {"title": "India policy change"}
    controller.state.script_data = {
    ...
    monkeypatch.setattr(
        ...
        lambda *_args: {"seconds": 31.0, "word_count": 78, "effective_wpm": 150.0},
    )

    reviewed = dict(controller.snapshot()["script_data"])
    reviewed["script"] = [dict(scene) for scene in reviewed["script"]]
