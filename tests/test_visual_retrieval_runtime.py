...
def test_person_action_reserves_action_search_even_when_exact_query_fills_target(monkeypatch):
    ...
    def provider(query, *args):
        return [
            {
                **_licensed_candidate(
                    _jpeg_bytes(color=(30 + index, 80, 120)),
                    "cc0",
                ),
                "source_image_url": f"https://example.test/exact/{index}.jpg",
                "search_title": "Vaibhav Sooryavanshi",
            }
    ...
    monkeypatch.setattr(
        retrieval,
        "_source_plan",
        lambda *args: [("Commons", provider), ("Openverse", provider)],
    )
    monkeypatch.setattr(
        visual_qa,
        "strict_gemini_check_batch",
    ...
    def fake_refinement(*args, **kwargs):
        ...
        return {
            "assets": [
                {
                    "bytes": _jpeg_bytes(color=(140 + index, 90, 70)),
                    "hash": f"refined-{index}",
                    "source": "Commons",
                    "query": args[2],
                    "visual_type": "PERSON",
    ...
    result = retrieval.collect_manual_visual_pool(
        ...
        [{
            "primary_entity": "Vaibhav Sooryavanshi",
            "voiceover": "Vaibhav Sooryavanshi batting in the cricket match.",
            "visual_intent": "batting cricket action",
        }],
        ["Vaibhav Sooryavanshi"],
        "Test story",
    ...
    assert result["query_stats"][0]["verified"] == 10
    assert result["query_stats"][0]["qa_requests"] == 2
