...
def write_script(story_data, language_cfg, genre_key, conn, format_mode):
    ...
    sys_prompt = (
        ...
        f"5. VISUALS (CRITICAL): You act as Visual Director. For each scene, identify the 'primary_entity' (ONE specific person/thing). Define 'visual_intent' ('editorial_person', 'stadium_event', 'news_event', 'conceptual'). Provide a 'specific_search_prompt' optimized for image search (e.g. 'Joe Root batting celebration cricket', NOT just 'Joe Root'). If a person appears multiple times, strictly vary the search prompt (e.g. 'press conference', 'action shot', 'close up helmet').\n"
...
def fetch_scene_asset(seg, category, used_urls, used_image_hashes, video_title=""):
    primary_entity = safe_text(seg.get('primary_entity', 'none')).strip()
    visual_intent = safe_text(seg.get('visual_intent', 'conceptual')).strip()
    ...
    if not bg_img: 
        ...
        for i in range(1920):
            draw.line([(0, i), (1080, i)], fill=(15, 20 + int((i/1920)*30), 35 + int((i/1920)*50)))
    
    return bg_img, True, source_type

async def process_visuals_async(script_data, language_cfg, format_mode="regular"):
    print(f"\n🎨 Asynchronously Sourcing Unique Editorial Packages...")
