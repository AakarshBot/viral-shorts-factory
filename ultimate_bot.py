...
def write_script(story_data, language_cfg, genre_key, conn, format_mode):
    ...
    system_prompt=(
        ...
        "- Target about 22–27 seconds of natural narration; hard maximum 30 seconds. The factory measures actual TTS afterward, so do not pad or force exact length.\n"
        "- Every scene adds new useful information. No filler, repetition, CTA, retention bait, generic setup or production instructions.\n"
        "- Scene 1: target 10–12 words, hard maximum 14. Before returning JSON, count Scene 1 words and rewrite it internally if it exceeds 14. Keep it as one crisp factual hook; do not stack context into the opener.\n"
        "- Before returning JSON, silently preflight: Scene 1 is within 14 words, narration targets roughly 22–27 seconds, every claim is supported, every scene adds new information, and exactly 3 titles are present. Fix any violation before output.\n"
        "- Middle scenes explain key evidence/context. Final scene delivers the consequence, significance or most useful closing fact.\n"
